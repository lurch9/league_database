...
def edit_score(request, score_id=None):
    score = get_object_or_404(Score, pk=score_id) if score_id else None
...
def search_player(request):
    ...
    if query:
        players = Player.objects.filter(firstname__icontains=query) | Player.objects.filter(lastname__icontains=query)
        if players.exists():
            return render(request, 'player_stats.html', {'players': players})
        else:
            return render(request, 'player_stats.html', {'error': 'No results found for this query.'})
    return render(request, 'player_stats.html', {'error': 'Please enter a search term.'})
...
def players_by_team(request):
    ...
    return render(request, 'league/players_by_team.html', {'error': 'No team name provided.'})
